if(!isDedicated) then {
	...
	player_spawnCheck			= compile preprocessFileLineNumbers "custom\fixes\player_spawnCheck.sqf";
	...
	player_removeObject			= compile preprocessFileLineNumbers "custom\a_plot_for_life\action\remove.sqf";
	...
	if(DZE_modularBuild) then {
		...
		DZE_snap_build_file 		= "custom\snap_pro\snap_build.sqf"; 
	...
	if(DZE_ConfigTrader) then {
		...
		call compile preprocessFileLineNumbers "custom\gold_coins\player_traderMenuHive.sqf";
	};

	call compile preprocessFileLineNumbers "custom\fixes\player_murderMenu.sqf";

	[] spawn {
		private["_timeOut","_display","_control1","_control2","_loadstatus","_maxtimeOut"];
