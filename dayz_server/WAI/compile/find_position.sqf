if(isServer) then {
	...
	while{!_validspot} do {
		...
		if(_validspot && wai_avoid_traders != 0) then {
			...
			{
				if((_position distance _x < wai_avoid_traders)) exitWith { if(debug_mode) then {diag_log("WAI: Invalid Position (Marker: " + str(_x) + ")");}; _validspot = false; };
			} count [[6326.48,7809.48],[4064.22,11665.93],[11447.91,11364.53],[9908.5,5446.78],[12944.22,12766.89],[1606.64,7803.52]];
		};
