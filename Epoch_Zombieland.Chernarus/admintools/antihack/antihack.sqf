private ["_debug","_lastpos","_lastheight","_lasttime","_lastVehicle","_curpos","_distance","_acceptableDistance","_curtime","_difftime","_curheight","_speed","_topSpeed","_terrainHeight","_safetyVehicle"];
		...
		if (((_distance > _acceptableDistance) || {(_difftime > 1)}) && (!((getPlayerUID player) in tempList))) then {
			...
			if (_lastVehicle == vehicle player) then {
				if ((_speed > _topSpeed) && (alive player) && ((driver (vehicle player) == player) || (isNull (driver (vehicle player)))) && (_debug distance _lastpos > 3000) && !((vehicle player == player) && (_curheight < _lastheight) && ((_curheight - _terrainHeight) > 1))) then {
					...
					PVDZE_atp = format["[TELEPORT REVERT] %6 (%1) from %2 to %3, %4 meters, now at %5",getPlayerUID player,_lastpos, _curPos,round(_lastpos distance _curpos),getPosATL player,name player];
					publicVariableServer "PVDZE_atp";
				} else {
					_lastpos = _curpos;
					_lastheight = _curheight;
